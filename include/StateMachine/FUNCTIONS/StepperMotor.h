...
public:
  // Constructor
  StepperMotor(uint8_t step, uint8_t dir, float stepsPerInch, long maxSpd,
               long maxAcc);
  ...
  void overrideSpeed(
      long speed); // Change speed during movement (smooth deceleration)
  void step();
  void moveSteps(long steps);
  void moveStepsSmooth(long steps); // Transitions smoothly from continuous mode
                                    // without force stop
  ...
  // pin: Enable pin number
  // active_low: true if pin should be LOW to enable (default true)
