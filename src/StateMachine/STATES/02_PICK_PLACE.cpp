...
extern int selectedPosition1Height; // Position 1 height selection (1-8, where 8
                                    // = a8/lowest)
extern int
...
extern int
    currentTestAllHeight; // Track which height we're currently testing (1-8)
extern int testAllColumnCount; // Number of columns to test (1-6)
extern int
    testAllStartColumn; // Starting column position (from physical position)
extern int testAllCurrentColumnIndex; // Current column index in the test
                                      // sequence (0 to testAllColumnCount-1)
...
void pickPlaceState() {
  ...
  if (cycleCancelled) {
    ...
    if (motorPaintRotation) {
      motorPaintRotation->stopContinuous(); // Was stopStepper()
  ...
  // This handles the case where painting state cancelled and we're back in
  // pick_place
  ...
  if (!testStarted) {
    ...
    // Turn on pressure pot when starting run cycle (unless test mode is
    // enabled)
    ...
    if (currentColumn < 0) {
      Serial.println(
          "[PICK_PLACE] Storage motor not homed - homing before run cycle...");
  ...
  if (step == 0) {
    ...
    float actualPos1Y = testPos1Y + (8 - selectedPosition1Height) *
                                        POSITION_HEIGHT_SPACING_INCHES;

    // Calculate target absolute positions (negate because positive direction
    // moves toward home switches)
  ...
  //! ************************************************************************
    ...
    float currentFork =
        motorFork->stepsToInches(motorFork->getCurrentPosition());

    // Calculate target absolute position (negate because positive direction
    // moves toward home switches)
  ...
  //! ************************************************************************
    ...
    // Set speed and acceleration for 0.5 inch movement (going up - use reduced
    // speed)
    motorY->setSpeed(
        (long)(TEST_Y_SPEED_FORK_EXTENDED * Y_SPEED_UP_MULTIPLIER));
  ...
  //! ************************************************************************
    ...
    // Check if square is present (ultrasonic sensor) - only if square sensing
    // is enabled Check multiple times over 100ms - if square detected at any
    // point, proceed If no square present after all checks, skip to end of
    // position 4 (step 15)
  ...
  //! ************************************************************************
    ...
    float targetX = -testPos1X;
    float targetY =
        -actualPos1Y - 0.5; // 0.5 higher means more negative (subtract 0.5)
  ...
  //! ************************************************************************
    ...
    // Skip return to 0 if we're in test all mode and have more heights or
    // columns to test
    if (testAllMode && (currentTestAllHeight < 8 ||
                        testAllCurrentColumnIndex < testAllColumnCount - 1)) {
      step = 17;
    } else {
      // Get current positions
      float currentX = motorX->stepsToInches(motorX->getCurrentPosition());
    ...
    if (testAllMode) {
      ...
      if (currentTestAllHeight < 8) {
        ...
        if (testAllCurrentColumnIndex < testAllColumnCount - 1) {
          // Home X, Y, and Fork axes before moving to next column (preserve
          // column position)
