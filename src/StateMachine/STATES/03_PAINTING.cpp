...
void waitForMotors(StepperMotor *motor1, StepperMotor *motor2) {
  while ((motor1 && motor1->isMotorRunning()) ||
         (motor2 && motor2->isMotorRunning())) {
    updateServoMovement(); // Update servo while waiting
    updateOTA();
    if (cycleCancelled)
      return;
...
// Turn on paint gun
void turnOnPaintGun() { digitalWrite(PAINT_GUN_PIN, HIGH); }

// Turn off paint gun
void turnOffPaintGun() { digitalWrite(PAINT_GUN_PIN, LOW); }

// Turn on suction
void turnOnSuction() { digitalWrite(SUCTION_PIN, HIGH); }

// Turn off suction
void turnOffSuction() { digitalWrite(SUCTION_PIN, LOW); }
...
// Start moving to waiting position (5 inches right of position 3) -
// non-blocking
...
void updateServoMovement() {
  ...
  float degreesPerMs =
      servoSpeed / 1000.0; // Convert to degrees per millisecond
  ...
  if (abs(diff) > SERVO_FAR_FROM_TARGET_THRESHOLD_DEG &&
      maxMovement < SERVO_MIN_MOVEMENT_DEG) {
    maxMovement = SERVO_MIN_MOVEMENT_DEG; // Minimum movement to ensure progress
  }

  // Calculate direction and movement
  float movement = (abs(diff) < maxMovement)
                       ? diff
                       : (diff > 0 ? maxMovement : -maxMovement);
  ...
  if (currentServoAngle < 0)
    currentServoAngle = 0;
  if (currentServoAngle > 270)
    currentServoAngle = 270;
...
void paintingState() {
  ...
  if (cycleCancelled) {
    ...
    if (motorPaintRotation) {
      ...
      motorPaintRotation->disableOutputs();
    }
    ...
    // Reset flags and return to idle (don't reset cycleCancelled here - let
    // pick_place handle it if needed) But since we're going to idle, we should
    // reset it
    cycleCancelled = false;
  ...
  //! ************************************************************************
    ...
    bool motorsRunning = (motorX && motorX->isMotorRunning()) ||
                         (motorY && motorY->isMotorRunning());
    while (motorsRunning) {
      updateServoMovement();
      updateOTA();
      if (cycleCancelled)
      ...
      motorsRunning = (motorX && motorX->isMotorRunning()) ||
                      (motorY && motorY->isMotorRunning());
      delay(1);
    }
  ...
  //! ************************************************************************
  //! STEP 3: MOVE SERVO TO PAINTING ANGLE AND START INITIAL 180-DEGREE ROTATION
  //! (SIMULTANEOUS)
  //! ************************************************************************
  else if (step == 2) {
    ...
    if (paintMotorStartPosition == 0) {
      ...
      if (motorPaintRotation) {
        ...
        motorPaintRotation->enableOutputs();
        motorPaintRotation->setCurrentPosition(
            0); // Reset to 0 to ensure clean start
        paintMotorStartPosition =
            motorPaintRotation->getCurrentPosition(); // Should be 0 now
      }
    }
    ...
    if (!initial180RotationStarted) {
      if (motorPaintRotation)
        motorPaintRotation->moveSteps(
            (long)(paintRotationMotorStepsPerRevOutput *
                   STEP4_INITIAL_ROTATION_REV));
  ...
  //! ************************************************************************
  //! STEP 5: WAIT FOR INITIAL 180-DEGREE ROTATION TO COMPLETE, THEN CONTINUE TO
  //! LEFT
  //! ************************************************************************
  else if (step == 4) {
    // Wait for initial 180-degree rotation to complete (if it was started)
    if (initial180RotationStarted && motorPaintRotation &&
        motorPaintRotation->isMotorRunning()) {
      updateServoMovement();
      updateOTA();
      if (cycleCancelled)
        return;
  ...
  //! ************************************************************************
    ...
    // Rotate full rotation CW (1.0 revolutions clockwise instead of 90 degrees
    // CCW)
    if (!rotationToLeftStarted) {
      if (motorPaintRotation)
        motorPaintRotation->moveSteps(
            (long)(paintRotationMotorStepsPerRevOutput *
                   STEP5_LEFT_ROTATION_REV));
  ...
  //! ************************************************************************
    ...
    if (savedBackServoSpeed == 0.0) {
      ...
      servoSpeed = STEP10_BACK_SERVO_SPEED;
    }
    ...
    if (!backWaitComplete) {
      ...
      if (elapsedTime >= STEP10_BACK_SIDE_WAIT_MS) {
        ...
        // Start moving servo to back angle
        startServoMoveToAngle(STEP10_SERVO_BACK_ANGLE_DEG);
      }
    }
    ...
    // Wait for paint delay at back angle (300ms)
      ...
      if (elapsedTime >= STEP10_BACK_PAINT_DELAY_MS) {
  ...
  //! ************************************************************************
    ...
    if (!rotationToBackRightStarted) {
      if (motorPaintRotation)
        motorPaintRotation->moveSteps(
            (long)(paintRotationMotorStepsPerRevOutput *
                   STEP11_BACK_RIGHT_ROTATION_REV));
  ...
  //! ************************************************************************
  //! STEP 20: TURN OFF PAINT GUN FOR 500MS BEFORE FINAL FRONT PASS, THEN TURN
  //! BACK ON
  //! ************************************************************************
  else if (step == 18) {
    // Turn off paint gun
  ...
  //! ************************************************************************
  //! STEP 21: FINAL FACE COAT - MOVE SERVO TO 220°, WAIT 200MS, THEN HOME, THEN
  //! BACK
  //! ************************************************************************
  else if (step == 19) {
        ...
        if (elapsedTime >= STEP17_INITIAL_ANGLE_WAIT_MS) {
          ...
          if (!step17PaintMotorCWStarted) {
            if (motorPaintRotation)
              motorPaintRotation->moveSteps(
                  (long)(paintRotationMotorStepsPerRevOutput *
                         0.125)); // 45 degrees = 0.125 revolutions
          ...
          if (step17PaintMotorCWStarted && !step17PaintMotorCWComplete) {
            if (!motorPaintRotation || !motorPaintRotation->isMotorRunning()) {
              ...
              // Start clockwise rotation to return to original position (0.875
              // rev = 1.0 - 0.125)
              if (motorPaintRotation)
                motorPaintRotation->moveSteps(
                    (long)(paintRotationMotorStepsPerRevOutput *
                           0.875)); // 315 degrees = 0.875 revolutions
            }
          }
  ...
  //! ************************************************************************
    ...
    if (!final180RotationStarted) {
      if (motorPaintRotation)
        motorPaintRotation->moveSteps(
            (long)(paintRotationMotorStepsPerRevOutput * 0.5));
  ...
  //! ************************************************************************
  //! STEP 22: TURN OFF SUCTION, CLEANUP AND RETURN TO PICK AND PLACE STATE -
  //! SKIPPED FOR NOW
  //! ************************************************************************
  // else if (step == 22) {
  //     // Turn off suction
