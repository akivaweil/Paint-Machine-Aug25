...
void setMachineState(int newState) { currentState = (MachineState)newState; }

// Function to get current state (accessible from web manager)
int getMachineState() { return (int)currentState; }
...
void loop() {
  // Handle OTA updates (WiFi and firmware updates) - always allow during any
  // state
  updateOTA();
