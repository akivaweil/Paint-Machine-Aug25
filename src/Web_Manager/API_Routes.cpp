...
void setupAPIRoutes() {
    ...
    // API endpoint to get/set cycle position values
    server.on("/api/cycle/positions", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        if (request->hasParam("pos1X") || request->hasParam("pos1Y") || request->hasParam("pos1Fork") ||
            ...
            if (request->hasParam("pos1X")) {
                cyclePos1X = request->getParam("pos1X")->value().toFloat();
            }
            if (request->hasParam("pos1Y")) {
                cyclePos1Y = request->getParam("pos1Y")->value().toFloat();
            }
            if (request->hasParam("pos1Fork")) {
                cyclePos1Fork = request->getParam("pos1Fork")->value().toFloat();
            }
            if (request->hasParam("pos2X")) {
                cyclePos2X = request->getParam("pos2X")->value().toFloat();
            }
            if (request->hasParam("pos2Y")) {
                cyclePos2Y = request->getParam("pos2Y")->value().toFloat();
            }
            if (request->hasParam("pos2Fork")) {
                cyclePos2Fork = request->getParam("pos2Fork")->value().toFloat();
            }
            ...
            request->send(200, "text/plain", "OK");
            Serial.println("Web Request: Cycle positions updated");
        } else {
            // Return current values if no parameters provided
            String json = "{";
            json += "\"pos1X\":" + String(cyclePos1X) + ",";
            json += "\"pos1Y\":" + String(cyclePos1Y) + ",";
            json += "\"pos1Fork\":" + String(cyclePos1Fork) + ",";
            json += "\"pos1Height\":" + String(selectedPosition1Height) + ",";
            json += "\"column\":" + String(selectedColumn) + ",";
            json += "\"pos2X\":" + String(cyclePos2X) + ",";
            json += "\"pos2Y\":" + String(cyclePos2Y) + ",";
            json += "\"pos2Fork\":" + String(cyclePos2Fork);
            json += "}";
            request->send(200, "application/json", json);
        }
    });
