...
void initializeSensors() {
  ...
  // Initialize Storage Position Sensor (active HIGH with pulldown, 5ms
  // debounce)
  ...
  if (servo == nullptr) {
    ...
    servo->write(
        SERVO_HOME_ANGLE); // Set to servo home angle (default/idle position)
  }
...
void setServoAngle(float angle) {
  if (servo) {
    ...
    const float stepDelayMs =
        (stepSize / servoSpeed) * 1000.0; // Delay in milliseconds
...
bool isSquarePresent() {
  ...
  // Distance = (pulse duration * speed) / 2 (divide by 2 because sound travels
  // to object and back)
  float distanceCm = (pulseDuration * 0.034) / 2.0;

  // Return true if distance is less than threshold (square present)
  return (distanceCm > 0 &&
          distanceCm < ULTRASONIC_SQUARE_PRESENT_THRESHOLD_CM);
}

// Initialize motors
// Initialize motors
void initializeMotors() {
  // Initialize paint rotation motor
  if (motorPaintRotation == nullptr) {
    // PAINT_ROTATION_STEP_PIN etc are defined in Config/Pin_Definitions
    // We need to support enabling, so we use setEnablePin
    motorPaintRotation = new StepperMotor(
        PAINT_ROTATION_STEP_PIN, PAINT_ROTATION_DIR_PIN, 1.0,
        PAINT_ROTATION_MOTOR_SPEED, PAINT_ROTATION_MOTOR_ACCEL);
    motorPaintRotation->setEnablePin(PAINT_ROTATION_ENABLE_PIN,
                                     true); // Active LOW
    motorPaintRotation->disableOutputs();
  }

  if (motorX == nullptr) {
    motorX = new StepperMotor(X_STEP_PIN, X_DIR_PIN, X_STEPS_PER_INCH,
                              X_MAX_SPEED, X_MAX_ACCEL);
  }
  if (motorY == nullptr) {
    motorY = new StepperMotor(Y_STEP_PIN, Y_DIR_PIN, STEPS_PER_INCH,
                              Y_MAX_SPEED, Y_MAX_ACCEL);
  }
  if (motorFork == nullptr) {
    motorFork = new StepperMotor(FORK_STEP_PIN, FORK_DIR_PIN, STEPS_PER_INCH,
                                 FORK_MAX_SPEED, FORK_MAX_ACCEL);
  }
  if (motorStorage == nullptr) {
    motorStorage =
        new StorageMotor(STORAGE_STEP_PIN, STORAGE_DIR_PIN, STORAGE_ENABLE_PIN,
                         STEPS_PER_INCH, motorSpeedStorage, motorAccelStorage);
  }
}
...
String getSensorStatesJSON() {
  ...
  bool squarePresent =
      isSquarePresent(); // Ultrasonic sensor - true if distance < threshold
  ...
  json +=
      "\"storagePosition\":" + String(storagePosition ? "true" : "false") + ",";
