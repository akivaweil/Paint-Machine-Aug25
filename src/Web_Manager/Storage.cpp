...
// Preferences namespace for cycle sequence persistence
Preferences preferences;

// Save cycle position values to non-volatile storage
void saveCyclePositions() {
    preferences.begin("cycleSeq", false);
    preferences.putFloat("pos1X", cyclePos1X);
    preferences.putFloat("pos1Y", cyclePos1Y);
    preferences.putFloat("pos1Fork", cyclePos1Fork);
    preferences.putFloat("pos2X", cyclePos2X);
    preferences.putFloat("pos2Y", cyclePos2Y);
    preferences.putFloat("pos2Fork", cyclePos2Fork);
    preferences.putInt("pos1Height", selectedPosition1Height);
    preferences.putInt("selectedColumn", selectedColumn);
    preferences.end();
}

// Load cycle position values from non-volatile storage
void loadCyclePositions() {
    preferences.begin("cycleSeq", true);
    cyclePos1X = preferences.getFloat("pos1X", 0.0);
    cyclePos1Y = preferences.getFloat("pos1Y", 0.0);
    cyclePos1Fork = preferences.getFloat("pos1Fork", 0.0);
    cyclePos2X = preferences.getFloat("pos2X", 0.0);
    cyclePos2Y = preferences.getFloat("pos2Y", 0.0);
    cyclePos2Fork = preferences.getFloat("pos2Fork", 0.0);
