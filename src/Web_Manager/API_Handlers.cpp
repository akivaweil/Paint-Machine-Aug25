...
void handleRoot(AsyncWebServerRequest *request) {
  ...
  for (size_t i = 0; i < len; i += CHUNK_SIZE) {
    ...
    chunk.replace("STORAGE_MOTOR_STEPS_PER_CLICK_VALUE",
                  String(storageMotorStepsPerClick));
    chunk.replace("PAINT_ROTATION_MOTOR_STEPS_PER_CLICK_VALUE",
                  String(paintRotationMotorStepsPerClick));
...
void handleStorageClockwise(AsyncWebServerRequest *request) {
  ...
  Serial.println(
      "Web Request: Move storage motor clockwise with location finding");
}
    ...
    if (axis == "x") {
      ...
      if (request->hasParam("steps")) {
        ...
        if (motorPaintRotation) {
          ...
          // Move the set amount (blocking logic)
          ...
          request->send(400, "text/plain",
                        "Paint Rotation Motor not initialized");
        }
      } else {
        request->send(400, "text/plain", "Missing steps parameter");
      }
...
void handleHome(AsyncWebServerRequest *request) {
  if (request->hasParam("axis")) {
    ...
    if (axis == "x") {
      ...
      homeAllAxes(
          false); // false = normal homing (not boot), preserve column position
      request->send(200, "text/plain", "OK");
...
void handleTestPositions(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("pos1X") || request->hasParam("pos1Y") ||
      request->hasParam("pos1Fork") || request->hasParam("pos2X") ||
      request->hasParam("pos2Y") || request->hasParam("pos2Fork") ||
...
void handleTestAll(AsyncWebServerRequest *request) {
  if (request->hasParam("pos1X") && request->hasParam("pos1Y") &&
    ...
    request->send(200, "text/plain", "OK");
    Serial.printf("Web Request: Start test all sequence (a1-a8) for %d columns "
...
void handleTest(AsyncWebServerRequest *request) {
  if (request->hasParam("pos1X") && request->hasParam("pos1Y") &&
    ...
    if (request->hasParam("pos1Height")) {
      selectedPosition1Height =
          request->getParam("pos1Height")->value().toInt();
      // Validate range (1-8)
...
void handleMotorSettings(AsyncWebServerRequest *request) {
  if (request->hasParam("speedX") && request->hasParam("accelX") &&
      ...
      request->hasParam("speedPaintRotation") &&
      request->hasParam("accelPaintRotation") &&
    ...
    motorSpeedPaintRotation =
        request->getParam("speedPaintRotation")->value().toInt();
    motorAccelPaintRotation =
        request->getParam("accelPaintRotation")->value().toInt();
    ...
    if (request->hasParam("storageSteps")) {
      storageMotorStepsPerClick =
          request->getParam("storageSteps")->value().toInt();
    }

    // Get storage trim distance if provided
    if (request->hasParam("storageTrim")) {
      storageMotorTrimDistance =
          request->getParam("storageTrim")->value().toInt();
    }

    // Get paint rotation steps if provided
    if (request->hasParam("paintRotationSteps")) {
      paintRotationMotorStepsPerClick =
          request->getParam("paintRotationSteps")->value().toInt();
    }

    // Get paint rotation steps per rev output if provided
    if (request->hasParam("paintRotationRevOutput")) {
      paintRotationMotorStepsPerRevOutput =
          request->getParam("paintRotationRevOutput")->value().toInt();
    }
    ...
    json +=
        "\"paintRotationSteps\":" + String(paintRotationMotorStepsPerClick) +
        ",";
    json += "\"paintRotationRevOutput\":" +
            String(paintRotationMotorStepsPerRevOutput) + ",";
...
void handleSquareSensing(AsyncWebServerRequest *request) {
  if (request->hasParam("enabled")) {
    ...
    String json =
        "{\"enabled\":" + String(squareSensingEnabled ? "true" : "false") + "}";
    request->send(200, "application/json", json);
...
void handleTestMode(AsyncWebServerRequest *request) {
  if (request->hasParam("enabled")) {
    ...
    saveTestModeState();
    String json =
        "{\"enabled\":" + String(testModeEnabled ? "true" : "false") + "}";
    request->send(200, "application/json", json);
...
void handleSkipPainting(AsyncWebServerRequest *request) {
  if (request->hasParam("enabled")) {
    ...
    String json =
        "{\"enabled\":" + String(skipPaintingEnabled ? "true" : "false") + "}";
    request->send(200, "application/json", json);
...
void handleCycleState(AsyncWebServerRequest *request) {
  ...
  if (state == STATE_HOMING)
    stateStr = "HOMING";
  else if (state == STATE_PICK_PLACE)
    stateStr = "PICK_PLACE";
  else if (state == STATE_PAINTING)
    stateStr = "PAINTING";
